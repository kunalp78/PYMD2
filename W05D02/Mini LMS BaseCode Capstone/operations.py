...
def Create_Unit(unit_json_file,ID,type_of_unit,title,start_time,end_time,scheduled_date):
    ...
    try:
        new_unit=json.load(f)
        if d not in new_unit:
            new_unit.append(d)
            f.seek(0)
            f.truncate()
            json.dump(new_unit,f)
    except JSONDecodeError:
...
def Read_all_Units(unit_json_file):
    ...
    l=[]
    fp=open(unit_json_file)
    content=json.load(fp)
    for i in range(len(content)):
        d={
            "Unit ID": content[i]["ID"],
            "Type": content[i]["Type"],
...
def Update_Unit(unit_json_file,Unit_ID,detail_to_be_updated,new_detail):
    ...
    fp=open(unit_json_file, "r+")
    content=json.load(fp)
    for i in range(len(content)):
        if content[i]["Unit ID"]==Unit_ID:
            if content[i].get(detail_to_be_updated) is False:
                return False
            content[i][detail_to_be_updated]=new_detail
            fp.seek(0)
            fp.truncate()
